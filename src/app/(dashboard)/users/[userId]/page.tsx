...
const UserDetailPage: React.FC = () => {
  ...
  const getGenderLabel = (gender: SEX) => (gender === SEX.MALE ? "ذكر" : "أنثى");
              ...
              <Card className="border border-muted rounded-xl shadow-md">
                ...
                <CardContent className="p-6">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    {[
                      { label: "الاسم الأول", value: user.firstName, icon: UserIcon },
                      { label: "الاسم الأخير", value: user.lastName, icon: UserIcon },
                      { label: "البريد الإلكتروني", value: user.email, icon: Mail, ltr: true },
                      { label: "رقم الهاتف", value: user.phone, icon: Phone, ltr: true },
                      { label: "المحافظة", value: user.governorate, icon: MapPin },
                      { label: "الجنس", value: getGenderLabel(user.gender), icon: Users },
                    ].map((field, index) => (
