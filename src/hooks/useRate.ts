...
import { getAvgRateForProduct, getMyRateForProduct, rateProduct } from "@/api-client"
import { APIResponse, RateProductProps } from "@/types/hooks"
import { useAppContext } from "@/contexts/AppProvider"

export const useGetAvgRateForProduct = (productId: number) => {
  return useQuery({
    queryKey: ["product-avg-rate", productId],
    queryFn: () => getAvgRateForProduct(productId),
    enabled: !!productId,
    retry: false,
  })
}
...
export const useRateProduct = () => {
  ...
  const onSuccess = (
    ...
    queryClient.invalidateQueries({ queryKey: ["product-avg-rate", productId] })
